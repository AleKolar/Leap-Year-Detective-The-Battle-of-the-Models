...
### 🧪 Автотесты (pytest)
- Полный набор юнит-тестов для функции `is_leap_year`.
- Интеграционные тесты всех API-эндпоинтов.
- Тесты LLM Arena с mock-объектами.
...
4. Результаты сохраняются в `app.state.arena_last_results`.
...
│   │   └── llm_arena.py     # Эндпоинты арены LLM
│   ├── services/
...
- **Статистика** хранится в глобальной переменной для простоты; при перезапуске сервера сбрасывается.
- **Результат арены** сохраняется в `app.state.arena_last_results` – также обнуляется при рестарте. В продакшене можно заменить на Redis.
...
- **Lifespan** создаёт общую `aiohttp.ClientSession` и проверяет доступность API при старте.
