...
## 📥 📄 Экспорт результата битвы (новое)

Мы добавили функциональность для **скачивания результата последней LLM-битвы в удобном формате**.

### 🧠 Что это даёт

Теперь можно получить готовый файл с результатами, который включает:
